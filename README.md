...
**Campaign complete — 13 rounds, weeks 0–12, 271 observations, 96 weekly queries, 32 records set.**
Mean normalised score **0.9951** across the seven functions with defensible ceilings.
...
| F4 | 4 | deterministic | −4.0255 | **0.64014** | W10 | 5 | 0.6549 · best-ever | 0.99685 |
...
\* F2's true maximum cannot be usefully bounded — its seed "best" was itself an upward draw, and its own Week-3 replicate at the same input returned 0.5580. Its ceiling is marked speculative, so it is scored separately and carries a wide uncertainty whisker rather than being dropped from the report.
...
Every notebook opens with the same navigation banner stating where it sits (A/B/C/D) and what role it plays. `pipeline/04_consolidate_data.ipynb` writes `function_summary.csv` on every run, so the project state is always derived from the data rather than from a hand-maintained document.

## Documentation & Transparency

In the interest of making this research transparent, reproducible, and credible, the data set and the optimisation approach have been fully documented:

* [📖 Datasheet and Model Card](DATASHEET_AND_MODEL_CARD.md) — formal dataset specifications, sampling biases, and a model card detailing the pathology-aware suggestion engine's assumptions, failure modes, and ethical considerations.
...
**The deep dives (`08_casino`, `09_harmonic`, `09_transform`)** — forensic treatments of the two functions that resisted. Read in sequence they document a methodological arc: notebook 08 reaches a conclusion, and each 09 overturns it on the same data plus later weeks, for reasons stated precisely enough to check.

### Diagnostics (C)

Auxiliary intelligence and theoretical depth. Nothing here selects a query, by design — the banners explain why.
...
Together A4 and A5 make the same point from two directions: at 22–52 observations in 2–8 dimensions, unsupervised structure is fragile, and the honest output is an effect size with a p-value rather than a convincing-looking scatter plot.
...
# The raw challenge data is private and is not part of this repository.
...
All paths are relative, with environment-variable overrides (`BBO_DATA_DIR`, `BBO_CONSOLIDATED_CSV`, `BBO_FIGURE_DIR`). The raw data is private and never enters the repository; the result CSVs, submission strings and figures under `pipeline/` are committed deliberately, so the repository shows its results without it. All of them regenerate on re-execution.
